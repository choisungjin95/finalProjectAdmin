...
public class ProposalController {
	...
	public String proposalBoard(Model model, @RequestParam(value = "pageNum", defaultValue = "1") int pageNum) throws JsonProcessingException {
		String url="http://192.168.0.12:9090/projectdb/proposal/brManagement/totalRowNum";
		...
		String url1="http://192.168.0.12:9090/projectdb/proposal/brManagement";
	...
	public String proBoardDetail(Model model, int proNum) {
		String url="http://192.168.0.12:9090/projectdb/proposal/proBoardDetail/proGetinfo?proNum="+proNum;
		String sproVo=service.get(url).trim();
		Gson gson=new Gson();
		ProposalVo proVo=gson.fromJson(sproVo, ProposalVo.class);
		model.addAttribute("proVo",proVo);
		String url1="http://192.168.0.12:9090/projectdb/proposal/proBoardDetail/nextPro?proNum="+proNum;
		String snextVo=service.get(url1).trim();
		ProposalVo nextVo=gson.fromJson(snextVo, ProposalVo.class);
		model.addAttribute("proVo",proVo);
		model.addAttribute("nextVo",nextVo);
		String url2="http://192.168.0.12:9090/projectdb/proposal/proBoardDetail/prePro?proNum="+proNum;
	...
	public String rejectProposal(Model model, int proNum) {
		String urlUpdate="http://192.168.0.12:9090/projectdb/proposal/rejectProposal?proNum="+proNum;
	...
	public String approveProposal(Model model, ProposalVo proVo) {
		...
		String urlEmail="http://192.168.0.12:9090/projectdb/proposal/approved/email?memId="+brVo.getMemId();
		String semail=service.get(urlEmail).trim();
		Gson gson=new Gson();
		String email=gson.fromJson(semail, String.class);
		String urlApprove="http://192.168.0.12:9090/projectdb/proposal/approved";
		...
		if(updateNum<1) {
			...
			model.addAttribute("proVo",proVo);
			String url1="http://192.168.0.12:9090/projectdb/proposal/proBoardDetail/nextPro?proNum="+proVo.getProNum();
			String snextVo=service.get(url1).trim();
			ProposalVo nextVo=gson.fromJson(snextVo, ProposalVo.class);
			model.addAttribute("proVo",proVo);
			model.addAttribute("nextVo",nextVo);
			String url2="http://192.168.0.12:9090/projectdb/proposal/proBoardDetail/prePro?proNum="+proVo.getProNum();
			String spreVo=service.get(url2).trim();
			ProposalVo preVo=gson.fromJson(spreVo, ProposalVo.class);
			model.addAttribute("proVo",proVo);
			model.addAttribute("nextVo",nextVo);
